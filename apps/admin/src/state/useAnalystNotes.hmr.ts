// HMR-only safety net: in dev, when `useAnalystNotes.ts` hot-reloads, accept
...
// suppression comments. Mirrors apps/admin/src/state/useResortDetail.hmr.ts.
//
// The accept target is `./useAnalystNotes` (a peer module, not this file's
// own module). When useAnalystNotes.ts changes, Vite re-evaluates it and the
// fresh empty Maps become the new module's initial state. Importing
// `./useAnalystNotes` here is forbidden — it would form a cycle with the
// side-effect import in useAnalystNotes.ts (per Codex round-8 P2-12); the
// dependency is expressed via the string literal in the accept call instead.

if (import.meta.hot) {
  import.meta.hot.accept('./useAnalystNotes', (): void => {
