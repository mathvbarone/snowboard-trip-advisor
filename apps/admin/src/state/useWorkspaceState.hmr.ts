...
// `useWorkspaceState.ts`. Excluded from coverage in apps/admin/vite.config.ts
...
// `useWorkspaceState.ts` (as of PR N.c3) registers a flusher into the GLOBAL
// `flushAll.ts` registry, which has NO HMR handling — its `flushers` Map
// survives a hot-reload of `useWorkspaceState.ts`. Without explicit
// deregistration the OLD module's flusher leaks: the NEW module lazily
// registers its own and `flushAllForSlug` (Promise.all) would invoke BOTH,
// letting a stale old store re-drive a PUT.
//
// Vite HMR ordering this relies on: when `useWorkspaceState.ts` changes,
...
//
// The accept target is `./useWorkspaceState` (a peer module, not this
// file's own module). Importing `./useWorkspaceState` here is forbidden —
// it would form a cycle with the value import in useWorkspaceState.ts (per
// Codex round-8 P2-12). The dependency is expressed via the string literal
// in the accept call instead, and the data flows the allowed direction only:
// `useWorkspaceState.ts` imports `registerHmrDisposer` from here; this file
 ...
 * `useWorkspaceState.ts` at module-evaluation time (including each HMR
...
if (import.meta.hot) {
  import.meta.hot.accept('./useWorkspaceState', (): void => {
