...
**Decision: Option 3 (bot-author exemption)** per [ADR-0009](../adr/0009-dco-exemption-for-dependabot.md). The `dco` job in `.github/workflows/ci.yml` skips DCO verification for commits whose author email matches a known Dependabot identity (exact-match, not substring). Adding a new bot (Renovate, Snyk, etc.) requires its own ADR plus an entry in the exemption list — the trust decision is not generalised silently.
