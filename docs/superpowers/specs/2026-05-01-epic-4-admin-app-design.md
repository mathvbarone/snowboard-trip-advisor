...
- **Tier 5 (Epic 4 done) gate (after 4.6a + 4.6b + 4.6c merged):**
  ...
  - Keyboard shortcuts work in browser smoke for the **4 shortcuts shipped in 4.6a** (`g r`, `g i` (Toast), `mod+enter` callback, `Escape` callback). The `/` shortcut is **deferred to Phase 2** when search functionality lands per [Tier 5 plan Decision B1](../plans/2026-05-12-epic-4-tier-5-closing-plan.md). `mod+enter` is fully wired to `useWorkspaceState.flushNow(slug)` on the editor route in PR 4.6c per [Tier 5 plan Decision K1](../plans/2026-05-12-epic-4-tier-5-closing-plan.md).
  - Responsive read-only enforced via DOM **native `disabled`** attribute on Shell header action buttons (NOT `tabindex="-1"+aria-disabled` per [Tier 5 plan Decision D1](../plans/2026-05-12-epic-4-tier-5-closing-plan.md) — design-system Button.tsx convention + WAI-ARIA 1.2 normative behavior) + render-gate-hidden edit input below `md` (NOT CSS-only; tested via simulated viewport). Shipped in PR 4.6a.
  - `useWorkspaceState` in-flight-clear race fixed via PR 4.6c (AbortController, path-gated by which paths were in the in-flight draft, per [Tier 5 plan Decision K1](../plans/2026-05-12-epic-4-tier-5-closing-plan.md)). The SPA's editor input + canonical correctly show the cleared state in-session after the user clears a value mid-PUT. Honest residual disk-side divergence documented inline in `useWorkspaceState.ts` and in §7.13 below — when the server has atomic-written before the abort propagates (likely on localhost), the workspace file on disk persists the typed value; the in-session UX is consistent, but a reload would re-surface the disk-side value. Recovery: re-edit or toggle AUTO.
...
Status: **DONE** — merged in [#95](https://github.com/mathvbarone/snowboard-trip-advisor/pull/95) (commit `07f2196`). Delivered: `apps/admin/src/state/useWorkspaceState.ts` (module-scoped per-slug `Map<ResortSlug, SlugStore>` singleton subscribed via `useSyncExternalStore`, 500ms debounce, in-flight token + revision counter + concurrent-PUT queue, draft reset + `prepopulateResortDetail` on PUT success per Decision D13, manual `FieldSource` provenance written on value edit per Decision D12) + `apps/admin/src/state/useModeToggle.ts` (silent no-op when path ∉ `validPaths`, since `eslint.config.js:90` blocks `console` in app code) + button-based `<ModeToggle>` (DS-Button-based, replacing 4.4b's inline `<span role="switch">` to comply with `RAW_HTML_ELS`) + MANUAL edit input via DS `Input type="text"` with JS-side numeric/range/integer validation for the 7 durable numeric paths + a minimal responsive read-only gate below the `md` breakpoint (Decision D11, polished in PR 4.6a) + `tests/integration/apps/admin/resort-editor-write.test.tsx` bridge integration asserting workspace files land on disk in a per-test tmpdir. **Spec deviations:** `useWorkspaceState` / `useModeToggle` derive `slug` internally via `useURLState` (Decision D7, avoids prop-drilling through `MetricPanel.tsx`); `<EditorErrorBoundary>` co-located inside `ResortEditor.tsx` rather than as a separate file (file budget); `useResortList.__resetForTests` remains unwired into `apps/admin/src/test-setup.ts` — pre-existing technical debt explicitly out-of-scope for Tier 3. **Phase-1 limitation — addressed in PR 4.6c (AbortController per [Tier 5 plan Decision K1](../plans/2026-05-12-epic-4-tier-5-closing-plan.md)):** the `useWorkspaceState` in-flight-clear race (Codex P2-B family, surfaced across 3 of the 4 Codex review rounds on PR #95) — see [`docs/superpowers/handoffs/2026-05-11-post-epic-4-tier-3.md`](../handoffs/2026-05-11-post-epic-4-tier-3.md) for the original reproducer. PR 4.6c extends `apiClient.upsertResort` with an optional `{ signal }`, threads an `AbortController` through `useWorkspaceState`'s flush path, and aborts in-flight PUTs ONLY when the cleared path was present in the in-flight draft (path-gated). The AbortError catch reverts in-flight-draft statuses to `dirty` (not `save-failed`), suppresses the response's prepopulate, and skips the `lastSentDraft` update. **Honest residual disk-side divergence:** when the server has atomic-written before the abort propagates (likely on localhost with sub-ms latency), the workspace file on disk persists the typed value while the SPA's editor input + canonical correctly show the cleared state in-session. Recovery: re-edit or toggle AUTO — the same user-facing recovery the pre-fix Phase-1 framing relied on, but now the in-session UX is consistent (no mismatched-input vs canonical surprise).
