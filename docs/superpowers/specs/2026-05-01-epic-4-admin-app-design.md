...
- **Editor** (PRs 4.4a-1, 4.4a-2, 4.4b, 4.4c, 4.4d — spec scoped quad-split on the server / view / write / interaction axes; PR 4.4a split into 4.4a-1 + 4.4a-2 at execution time for file-budget per §7.10): 4.4a-1 ships `projectFieldStates` + seed admin-workspace fixtures; 4.4a-2 ships the real `resortDetail` server handler + workspace read helpers + `useResortDetail` hook; 4.4b ships the read-only editor view (durable + live panels + StatusPill); 4.4c ships the server write path (`resortUpsert` + workspace atomic-write helper) — server-only, with the client-side `useWorkspaceState` hook shipped in 4.4d alongside the interactive ModeToggle; 4.4d makes the editor edit-interactive (ModeToggle flips AUTO↔MANUAL via `editor_modes`, MANUAL exposes edit input, integration write round-trip via `bridgeHandlers(tmpdir)`).
- **Publish** (PRs 4.5a, 4.5b, 4.5c, 4.5d — spec scoped dual-split on the handler / UI axis; PR 4.5b split into 4.5b + 4.5c + 4.5d at execution time for file-budget per §7.15): 4.5a ships `publish` + `listPublishes` server handlers; 4.5b ships the Toast DS primitive (first real consumer is publish success/failure); 4.5c ships `usePublish` + `useListPublishes` + `<PublishDialog>` + Shell wire-up (the 4-condition pre-publish blocking-state surface per §4.3.1, including `resorts_with_corrupt_workspace > 0`); 4.5d ships `<PublishHistory>` at `?route=publishes` + the bridge integration test (`publish-flow.test.tsx`).
- **Closing** (PRs 4.6a, 4.6b, 4.6c — spec scoped dual-split on the polish / integration-backfill axis; PR 4.6c split off PR 4.6a at execution time for file-budget per §7.17.1): 4.6a ships keyboard shortcuts + responsive read-only-below-md affordance; 4.6b ships the integration backfill (Dashboard, ResortsTable, full-flow); 4.6c ships the `useWorkspaceState` in-flight-clear race fix + `flushNow` + Shell `mod+enter` wiring. 4.6a and 4.6b are parallel-capable (no shared files); 4.6c is sequential after 4.6a (shares `Shell.tsx` + `useWorkspaceState.ts`). The Dockerfile prod-build guard is **deferred to Epic 6** alongside the Dockerfile rewrite (the existing `Dockerfile` is broken — see §10.7).
...
Tier 1–4 gates **CLOSED**; criteria preserved in git history at the merge commits cited in §7.5–§7.15:

- **Tier 1 → 2 gate** — CLOSED at PR #81 (PR 4.1c, commit `ee78d9c`).
- **Tier 2 → 3 gate** — CLOSED at PR #88 (PR 4.3.1, commit `4657df3`). Card-click + row-click navigation closed the deferred half (see §7.9.1).
- **Tier 3 → 4 gate** — CLOSED at PR #95 (PR 4.4d, commit `07f2196`).
- **Tier 4 → 5 gate** — CLOSED at PR #102 (PR 4.5d, commit `481f434`). Publish flow → on-disk `data/published/history/` growth + Toast + PublishHistory entry verified via bridge integration; all 4 §4.3.1 blocking conditions exercised.

**Tier 5 (Epic 4 done) gate — CLOSED at PR #105** (PR 4.6c, commit `910c5d1`):

- `full-flow.test.tsx` green (bridge tier — composite open admin → Resorts → row click → MANUAL edit → save → publish → see in history). Shipped in PR 4.6b.
- Keyboard shortcuts work in browser smoke for **4 of 5** spec §3.10 shortcuts (`g r`, `g i` (Toast), `mod+enter`, `Escape`). The `/` shortcut is **deferred to Phase 2** when search functionality lands — a focus-only placeholder `<input type="search">` would violate WCAG 3.3.2 + 4.1.2. `mod+enter` is fully wired to `useWorkspaceState.flushNow(slug)` on the editor route in PR 4.6c.
- Responsive read-only enforced via DOM **native `disabled`** attribute on Shell header action buttons (NOT `tabindex="-1"+aria-disabled` — per design-system [`Button.tsx:35-40`](../../../packages/design-system/src/components/Button.tsx) convention + WAI-ARIA 1.2 normative behavior; native `disabled` removes from tab order, prevents mouse activation, AND triggers `:disabled` CSS) + render-gate-hidden edit input below `md` (NOT CSS-only; tested via simulated viewport). Shipped in PR 4.6a.
- `useWorkspaceState` in-flight-clear race fixed via PR 4.6c (AbortController, path-gated by which paths were in the in-flight draft). The SPA's editor input + canonical correctly show the cleared state in-session after the user clears a value mid-PUT. Honest residual disk-side divergence documented inline in `useWorkspaceState.ts`, §7.13 below, and [`docs/superpowers/handoffs/2026-05-12-post-epic-4.md`](../handoffs/2026-05-12-post-epic-4.md) — when the server has atomic-written before the abort propagates (likely on localhost), the workspace file on disk persists the typed value; the in-session UX is consistent, but a reload would re-surface the disk-side value. Recovery: re-edit or toggle AUTO.
...
Status: **DONE** — merged in [#95](https://github.com/mathvbarone/snowboard-trip-advisor/pull/95) (commit `07f2196`). Delivered: `apps/admin/src/state/useWorkspaceState.ts` (module-scoped per-slug `Map<ResortSlug, SlugStore>` singleton subscribed via `useSyncExternalStore`, 500ms debounce, in-flight token + revision counter + concurrent-PUT queue, draft reset + `prepopulateResortDetail` on PUT success per Decision D13, manual `FieldSource` provenance written on value edit per Decision D12) + `apps/admin/src/state/useModeToggle.ts` (silent no-op when path ∉ `validPaths`, since `eslint.config.js:90` blocks `console` in app code) + button-based `<ModeToggle>` (DS-Button-based, replacing 4.4b's inline `<span role="switch">` to comply with `RAW_HTML_ELS`) + MANUAL edit input via DS `Input type="text"` with JS-side numeric/range/integer validation for the 7 durable numeric paths + a minimal responsive read-only gate below the `md` breakpoint (Decision D11, polished in PR 4.6a) + `tests/integration/apps/admin/resort-editor-write.test.tsx` bridge integration asserting workspace files land on disk in a per-test tmpdir. **Spec deviations:** `useWorkspaceState` / `useModeToggle` derive `slug` internally via `useURLState` (Decision D7, avoids prop-drilling through `MetricPanel.tsx`); `<EditorErrorBoundary>` co-located inside `ResortEditor.tsx` rather than as a separate file (file budget); `useResortList.__resetForTests` remains unwired into `apps/admin/src/test-setup.ts` — pre-existing technical debt explicitly out-of-scope for Tier 3. **Phase-1 limitation — resolved in PR 4.6c:** the `useWorkspaceState` in-flight-clear race (Codex P2-B family, surfaced across 3 of the 4 Codex review rounds on PR #95) was addressed via an AbortController approach (path-gated by which paths were in the in-flight draft). See [§7.17.1](#7171-pr-46c--useworkspacestate-race-fix--flushnow--shell-modenter-wiring-tier-5-split-off) for the resolution narrative + the honest residual disk-side divergence note, and the post-Epic-4 handoff at [`docs/superpowers/handoffs/2026-05-12-post-epic-4.md`](../handoffs/2026-05-12-post-epic-4.md) for the Phase 2 watch-item.

### 7.14 PR 4.5a — Publish handler + listPublishes handler

Status: **DONE** — merged in [#99](https://github.com/mathvbarone/snowboard-trip-advisor/pull/99) (commit `b5dd1b4`). Delivered: real `apps/admin/server/publish.ts` (replaces 4.1b's 501 stub) composing the publish input as the **workspace ∪ published union** per §4.6 (workspace overrides per slug; published-only resorts kept as-is) → `PublishedDataset` → `publishDataset()` from `@snowboard-trip-advisor/schema/node` (direct library call, NOT a subprocess — same library the Epic 2 CLI exercises) with the Phase-1 `slug === '__all__'` sentinel assertion. Real `apps/admin/server/listPublishes.ts` parses `${counter}-${iso}.json` history filenames newest-first per §4.7. `apps/admin/src/lib/apiClient.ts` MODIFIED to inject `Idempotency-Key: ${crypto.randomUUID()}` into every publish POST per §4.9 invariant 5 — Phase-1 "honor without enforce" (`DispatchInput` has no `headers` field today; the application server never observes the header in Phase 1; Phase 2 Hono swap will add header plumbing for real dedupe). Server delegates all blocking validation to `publishDataset()`; no `publish-blocked-*` codes added — the existing `publish-validation-failed` envelope is the canonical failure code.

### 7.15 PR 4.5b/c/d — Publish UI (Toast + state hooks + dialog + history view)

Status: **DONE** — shipped as **three PRs** (spec originally scoped one PR 4.5b; execution split the UI tier per the ≤8-file-budget convention, mirroring §7.10's PR 4.4a split):

- **4.5b** ([#100](https://github.com/mathvbarone/snowboard-trip-advisor/pull/100), commit `b27a28a`) — Toast DS primitive with 3 variants (`info` / `success` / `error`), `role="status"` for info+success and `role="alert"` for error, per-variant auto-dismiss timing (5s / 5s / 8s), pause-on-hover + pause-on-focus + visible Dismiss button for WCAG 2.2 SC 2.2.1 keyboard parity. Single-slot Provider (no queue) — Phase 2 adapter actions revisit if concurrent Toasts emerge. Plus a `Button` ARIA extension consumed by the Dismiss surface.
- **4.5c** ([#101](https://github.com/mathvbarone/snowboard-trip-advisor/pull/101), commit `0be1d98`) — `apps/admin/src/state/usePublish.ts` (callback-based mutation hook returning `{submit, status, response, error, reset}`; calls `apiClient.publish()` + `invalidateListPublishes()` on success — NOT Suspense-based, since publish is a mutation, not a render-time read) + `apps/admin/src/state/useListPublishes.ts` (`useState + useEffect + module-level in-flight `Map` cache + per-key subscriber set` matching `useResortList`'s actual pattern — **NOT** Suspense-based; exposes `invalidateListPublishes()` module export and `__resetForTests()`) + `apps/admin/src/views/PublishDialog.tsx` (wraps the existing DS `Modal` primitive at `packages/design-system/src/primitives/Modal.tsx` for focus trap + scroll lock + Escape-dismiss + portal; consumes `useHealth()` for the 4 blocking-state surface per §4.3.1; `aria-describedby` wires the disabled-reason `<p>` for AT users in all 6 disabled states — health-loading + health-error + the 4 blockers) + Shell wire-up (Publish `<Button>` + conditional-mount dialog so each open re-runs `useHealth()`'s effect against the latest snapshot).
- **4.5d** ([#102](https://github.com/mathvbarone/snowboard-trip-advisor/pull/102), commit `481f434`) — `apps/admin/src/views/PublishHistory.tsx` route at `?route=publishes` (sorted desc by `published_at`, page size 20 matching `ListPublishesQuery` default; `urlState.ts` MODIFY adds `{ route: 'publishes'; page?: number }`) + `tests/integration/apps/admin/publish-flow.test.tsx` bridge integration (per-test workspace + history tmpdir; asserts both SPA state AND on-disk `data/published/history/` growth; exercises happy path + each of the 4 blocking conditions + forced failure).

**Spec deviations.** `useHealth` extension (subscription via `useSyncExternalStore` mirroring `useResortDetail`'s round-2 P2-C pattern) was dropped per Codex round-2 fold on PR #97 — would have pushed PR 4.5c over the file budget without buying load-bearing behavior, since PublishDialog re-fetches health per open via its existing mount effect. Dashboard's persistent health-card display lags one navigation cycle behind a publish (acceptable Phase-1 limitation; see post-Epic-4 handoff for the Epic 5 watch-item).

### 7.16 PR 4.6a — Polish (keyboard shortcuts + responsive read-only-below-md)

Status: **DONE** — merged in [#103](https://github.com/mathvbarone/snowboard-trip-advisor/pull/103) (commit `857bb0c`). Delivered: `apps/admin/src/lib/shortcuts.ts` (single document-level `keydown` listener; handlers pinned via `useRef` so re-renders with fresh inline handler closures don't tear down + re-attach the listener; `g _` chord debounce 1000 ms; editable-target bypass for `g _` when active element is INPUT / TEXTAREA / SELECT / contenteditable; cross-platform `mod+enter` covers Meta+Enter on macOS and Control+Enter elsewhere) + `apps/admin/src/lib/useResponsiveTabOrder.ts` (returns `{ readOnly: boolean }` keyed off `(min-width: ${tokens.breakpoint.md}px)` via `useSyncExternalStore` over `window.matchMedia`; jsdom-friendly `readOnly: false` fallback when matchMedia is unavailable) + `apps/admin/src/views/Shell.responsive.css.ts` (media-query overlay for header padding + brand-text hide below `md`) + `Shell.test.tsx` (NEW; tab-order discipline + RESPONSIVE_CSS injection assertions) + `FieldRow.tsx` MODIFY (consumes `useResponsiveTabOrder`; renders read-only `<span>` instead of edit input when `readOnly === true`; drops the inlined `useIsAboveMd` / `MD_QUERY` / `tokens` / `useSyncExternalStore` imports from PR 4.4d D11) + `Shell.tsx` MODIFY (wires `useShortcuts({onGoResorts, onGoIntegrations, onModEnter, onEscape})`; applies native `disabled={readOnly}` to the Publish `<Button>` + the inner `<Button>` of the Account `<DropdownMenu>` trigger — preserved through Radix's `cloneElement` merge; mounts `<style>{RESPONSIVE_CSS}</style>` at the Shell root; `g i` surfaces a Toast "Integrations management isn't available yet."; `mod+enter` and `Escape` ship as Phase-1 no-ops — `mod+enter` real flush wires in PR 4.6c; Radix Dialog handles modal Escape internally, so the callback is forward-compat for Phase 2 non-Radix overlays).

**Ships 4 of the 5 spec §3.10 shortcuts.** The `/` shortcut is **deferred to Phase 2** when search functionality lands — a focus-only placeholder `<input type="search">` would violate WCAG 3.3.2 + 4.1.2 (the input claims a search role, does nothing; assistive-tech users typing get silence, no autocomplete, no submit feedback).

**Co-ships a load-bearing spec amendment.** The §7.4 done-gate criterion 2 + this §7.16 paragraph mandate **native `disabled={readOnly}`** on header action buttons (NOT `tabIndex={-1}` + `aria-disabled` as the earlier spec wording prescribed) — per [`packages/design-system/src/components/Button.tsx:35-40`](../../../packages/design-system/src/components/Button.tsx) ("we deliberately do NOT add a parallel `aria-disabled` prop — the native `disabled` attribute already conveys the disabled state to assistive tech") AND WAI-ARIA 1.2 normative behavior ("aria-disabled does not change operability — the element is still focusable, perceivable, and operable to AT and to mouse"). Native `disabled` removes from tab order, prevents mouse activation, AND triggers `:disabled` CSS for visual state. The amendment co-shipped with the code change per AGENTS.md §95 documentation discipline; Subagent Review fired per AGENTS.md §60 since `docs/superpowers/specs/**` is a CODEOWNERS-protected path.

### 7.17 PR 4.6b — Integration backfill (closing PR)

Status: **DONE** — merged in [#104](https://github.com/mathvbarone/snowboard-trip-advisor/pull/104) (commit `de8b2be`). Delivered: `tests/integration/apps/admin/dashboard.test.tsx` (canned tier per §6.3 — health cards render + click-through-to-filtered-resorts; cold-start empty state per §10.9) + `tests/integration/apps/admin/resorts-table.test.tsx` (canned tier — sort + filter + row-click URL update; cold-start empty-state row) + `tests/integration/apps/admin/full-flow.test.tsx` (bridge tier — per-test workspace + history tmpdir; composite open admin → navigate to Resorts → row click → MANUAL edit → save → publish → see in PublishHistory; asserts both SPA-visible state AND on-disk filesystem state). Dockerfile prod-build guard remains deferred to Epic 6 per §10.7 / C4.

### 7.17.1 PR 4.6c — `useWorkspaceState` race fix + `flushNow` + Shell `mod+enter` wiring (Tier 5 split-off)

Status: **DONE** — merged in [#105](https://github.com/mathvbarone/snowboard-trip-advisor/pull/105) (commit `910c5d1`). Split off from PR 4.6a at Tier 5 planning time because the carryforward `useWorkspaceState` in-flight-clear race fix (originally documented as a §7.13 Phase-1 limitation) would have pushed PR 4.6a over the ≤8-file budget AND mixed two concerns (polish + race-fix) in one PR. Delivered: the race fix per the chosen **AbortController** architectural approach (rejected the alternative `null`-as-clear PUT-API widening because it would touch `packages/schema/**` and invert the schema's required-field nominal contract). `apps/admin/src/lib/apiClient.ts` MODIFIED to thread an optional `{ signal }` through `apiClient.request()` and `apiClient.upsertResort()`. `apps/admin/src/state/useWorkspaceState.ts` MODIFIED — `SlugStore` gains `abortController` + `inFlightDraft` fields; `flush()` creates a fresh controller, passes the signal, catches `AbortError` (`err instanceof DOMException && err.name === 'AbortError'`) on a distinct branch that reverts in-flight-draft path statuses to `dirty` (not `save-failed`), skips the response's prepopulate, and skips the `lastSentDraft` update; `clearFieldValue` reads `store.inFlightDraft` and aborts the controller **only when the cleared path was present in the in-flight draft** (path-gated, not always); `__resetForTests()` aborts every controller before clearing; the AbortError catch no-ops when `storesBySlug.get(slug) !== store` so reset-mid-flight doesn't push stale state through subscribers. Adds a `flushNow(slug)` synchronous-save shortcut export. `apps/admin/src/views/Shell.tsx` MODIFIED to read `useURLState()` and wire `onModEnter` to `flushNow(route.slug)` on the editor route (off-route stays no-op).

**Honest residual disk-side divergence.** When the server has atomic-written *before* the abort propagates (likely on localhost with sub-ms latency), the workspace file on disk persists the typed value while the SPA's editor input + canonical correctly show the cleared state in-session. Recovery: re-edit or toggle AUTO — the in-session UX is now fully consistent (no mismatched-input vs canonical surprise), and the reload-divergence is a documented Phase-1 limitation. See [`docs/superpowers/handoffs/2026-05-12-post-epic-4.md`](../handoffs/2026-05-12-post-epic-4.md) for the Phase 2 watch-item.
