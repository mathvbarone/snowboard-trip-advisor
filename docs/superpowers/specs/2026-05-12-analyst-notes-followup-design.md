...
3. Acquire `withSlugLock(slug, async () => { ... })`:
   ...
   - **No-op delete short-circuit:** if `body.markdown === null && workspaceFile === null` (delete request against a published-only resort that has no workspace file), there is no note to remove. Return `{ slug, path, note: null }` immediately, BEFORE the cold-start construction below. Without this short-circuit, the cold-start branch would materialize a fresh `WorkspaceFile` copying the published `resort` / `live_signal` values with an empty `notes: {}`, the patch step's `delete wf.notes[path]` would be a no-op (key already absent), and `atomicWriteWorkspaceFile` would still write that empty-notes snapshot to disk. The snapshot then *shadows* future publish-dataset updates for this slug (the GET handler prefers workspace over published per §3.2 step 2), which is a phantom state-divergence for a resort the analyst hadn't actually touched. Treating the absent-note delete as the no-op it is keeps the published-only resort published-only.
...
Plus `markdown.fuzz.test.ts` (fast-check): for any random string input, **parses the rendered HTML and asserts on the AST** (not on raw substring presence):
- No `<script>` element exists in the parsed tree.
...
1. **`setDraft(markdown)`** — `state.draft = markdown; state.status = 'dirty'; state.rev++; state.lastFlightKind = 'upsert'` (typed text always represents an upsert intent — clears any stale `'delete'` kind left over from a previous failed-delete flight so the §5.2 retry routing doesn't false-positive on the next flush). Cancel previous debounce timer; schedule new at 500 ms. Notify subscribers.
2. **Flush (debounce OR `flushNow()`):**
   ...
   - Structural equality short-circuit: if `(state.draft === state.lastSent || (state.lastSent === null && state.draft === '')) && state.status !== 'save-failed' && state.status !== 'saving'` → set `state.status = 'saved'` (the `dirty` flag set by step 1 was speculative; once we observe equality, there is nothing to save — leaving status at `'dirty'` would strand the UI's status indicator on a perpetual unsaved-changes signal — mirrors [`useWorkspaceState.ts:391`](../../../apps/admin/src/state/useWorkspaceState.ts)'s empty-diff short-circuit that calls `patchState(..., 'saved')` for the same reason), notify subscribers, and return. **Excluding `'saving'` closes the reverted-draft race**: if an upsert of value A is in flight (`status === 'saving'`) and the analyst reverts the textarea to `lastSent === B` before A's response lands, a `flushNow()` / debounce / `mod+enter` would observe `draft === lastSent` and (without the `'saving'` exclusion) mark status `'saved'` and return. But the in-flight PUT for A is still allowed to land on the server, the rev guard then skips the prepopulate, and the workspace file ends up holding A while the UI claims saved-at-B — `lastSent`-invariant violation, durable state divergence. Falling through to the flush body in that case aborts the in-flight (controller-identity guard handles the abort safely) and sends a fresh PUT for the current (`= lastSent`) draft — one extra round-trip, but server-confirmed convergence. The first disjunct is the normal "no pending change" case; the second handles the post-delete baseline. After `deleteNote()` succeeds, step 3 leaves `lastSent === null` (note removed server-side, nothing to compare against) and `draft === ''` (textarea cleared). Without the disjunct, the *next* flush trigger — `flushNow()`, the 500 ms debounce, or `flushAllForSlug` via Shell mod+enter — would observe `'' !== null`, fall through, send `PUT { markdown: '' }`, and immediately recreate the just-deleted note as an empty upsert (per the spec §3.1 semantics `markdown: ''` is upsert-empty, NOT delete). The second-disjunct guard fires only when both halves match the deleted-baseline shape; the regular dirty-resurrection path (analyst types after delete → `draft = 'x'`, `lastSent = null`, `'x' !== null` → flush proceeds and PUTs `{ markdown: 'x' }`) is unaffected.
   ...
   - On success:
     ...
     - Else: `state.lastSent = state.draft; state.status = 'saved'; state.lastFlightKind = null`. **Controller-identity guard:** `if (state.abortController === localController) state.abortController = undefined` (else a fresh `deleteNote` / `setDraft` already installed a newer controller — do not clobber it). Then `prepopulateAnalystNotes(slug, merge(slug, response.path, response.note))`. (Do NOT reassign `state.draft` — the analyst may keep editing the same path; preserving the textarea content is the natural UX.)
...
3. **`deleteNote()`:**
   - **Cancel any pending debounce timer first**: `if (state.debounceTimer !== undefined) { clearTimeout(state.debounceTimer); state.debounceTimer = undefined }`. Without this, a typing → Delete sequence inside the 500 ms debounce window would leave the original `setDraft`'s timer armed. The timer would then fire while the delete PUT is in flight and re-enter step-2 flush; flush would observe `draft === ''` AND `lastSent !== null` (per R8.1 the delete keeps `lastSent` at the pre-delete value), neither short-circuit disjunct matches, the failed-delete retry routing doesn't match either (status is `'saving'`, not `'save-failed'`), so flush falls through, aborts the in-flight delete via `state.abortController?.abort()`, and sends `PUT { markdown: '' }` — recreating/replacing the note as an empty upsert instead of deleting it.
   ...
   - `state.draft = ''; state.status = 'saving'; state.lastFlightKind = 'delete'`. **Do NOT mutate `state.lastSent` here** — the server still holds the original note until the delete confirmation lands. Pre-emptively clearing it would violate the §5.1 invariant (`lastSent === server's current markdown OR null iff no note`) on a failed-delete state, leaving the hook in a no-note baseline even though the server still has the note. `lastSent` is set to `null` only by the success branch below, when the server confirms the deletion. (`lastFlightKind = 'delete'` is the field §5.2 step 2's failed-delete retry routing keys on — it lets the failed-delete state remain detectable without overloading `lastSent` as a sentinel.)
   ...
   - On success:
     ...
     - Else: `state.status = 'saved'; state.lastSent = null; state.lastFlightKind = null` (server confirmed delete — invariant now holds with `lastSent` reflecting "no note exists"). **Controller-identity guard:** `if (state.abortController === localController) state.abortController = undefined`. Then `prepopulateAnalystNotes(slug, merge(slug, path, null))` (note removed from cache). Status terminal-after-success; next `setDraft` re-enters the upsert path with `lastSent === null`.
   - On `AbortError` (the delete itself got aborted by ANOTHER `setDraft`): leave status (the new `setDraft` already updated status); leave `state.abortController` alone (the new `setDraft` already installed its own); leave `state.lastSent` alone (server still has the original note); leave `state.lastFlightKind` alone (the new `setDraft` already flipped it to `'upsert'`).
   - On other error (rev-guarded the same way): `state.status = 'save-failed'`. `state.lastSent` remains at its pre-delete value (the server still holds that note); `state.lastFlightKind` remains `'delete'` so the §5.2 step 2 failed-delete retry routing can detect this state and dispatch the retry through `deleteNote()` rather than the upsert path.
4. **Unmount:** clean up subscriber; do NOT abort in-flight saves (analyst-walks-away convention); debounce timer lives on the module-level SlugStore (not the component), so the timer fires post-unmount and the flush proceeds without a consumer. Per §5.4, the slug-level `flushAll` registration also lives on the SlugStore — it is NOT deregistered when the component unmounts. So a typing → row-collapse → `mod+enter` sequence still flushes the dirty path through the §5.4 registry (closes the R9.1 race where a hook-scoped flusher would have deregistered itself on row collapse). Next mount reads the prepopulated value. Documented trade-off: navigation away has up to 500 ms tail latency before the debounce fires — `flushAllForSlug` on route-change is a Phase 2 question.
...
- **Registration sits on the SlugStore, not on the React hook**. The slug-level `useWorkspaceState` SlugStore and the slug-level `useAnalystNoteDraft` SlugStore each register a single `flushAll(): Promise<void>` method into `flushers.get(slug)` the first time the SlugStore is created (lazily, on first read for that slug). Deregistration only fires from the sibling `.hmr.ts` cleanup, not from any component unmount. The SlugStore's `flushAll` iterates every per-path state it owns and flushes the dirty / save-failed ones.
- **Why not on the hook's `useEffect` cleanup?** `AnalystNoteSection` is lazy-mounted only while a row is expanded. If the analyst types, collapses the row before the 500 ms debounce fires, and then presses `mod+enter`, a hook-scoped flusher would have already deregistered itself on the row's unmount — `flushAllForSlug` would miss the still-dirty path, the debounce timer would eventually fire post-unmount, but if the analyst navigates away in the meantime the save is lost. Anchoring registration on the (durable, module-level) SlugStore decouples flush plumbing from component lifecycle and keeps every dirty per-path state reachable to `flushAllForSlug` until the SlugStore itself goes away.
