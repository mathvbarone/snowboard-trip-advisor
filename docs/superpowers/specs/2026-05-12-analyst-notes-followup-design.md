...
2. **Flush (debounce OR `flushNow()`):**
   ...
   - **Failed-delete retry routing:** if `state.lastSent === null && state.draft === '' && state.status === 'save-failed'`, the failed flight was a `deleteNote()` — the flush path always sends an upsert, so without this branch the retry would `PUT { markdown: '' }` and resurrect the note as an empty upsert (per §3.1 semantics). Instead, dispatch through `deleteNote()` (re-acquires a fresh controller, re-sends `PUT { markdown: null }`, follows step 3's terminal handlers). Return after dispatching. Subsequent `setDraft('x')` calls remain on the upsert path naturally — once `state.draft !== ''`, this routing condition stops matching and the user's typed text retries as an upsert.
