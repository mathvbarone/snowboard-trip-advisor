...
| **J1 (PR 4.6c)** | **In-flight-clear race fix architectural approach: TBD at 4.6c kickoff.** Two candidates per [post-Tier-3 handoff §52](../handoffs/2026-05-11-post-epic-4-tier-3.md): **(a) AbortController** race against the in-flight PUT (suppresses the response's prepopulate when the user has cleared mid-flight) — heavier client logic; preserves schema's nominal contract. **(b) PUT-API widening** with `null`-as-clear semantic for required fields — contradicts schema's required-field contract; would require `ResortUpsertBody` schema change. Brainstorm with `superpowers:brainstorming` at PR 4.6c kickoff before coding. | Defer per A1 split. Maintainer accepted Phase-1 limitation when merging PR #95. |
...
| # | Path | Action |
|---|---|---|
| 1 | `apps/admin/src/state/useWorkspaceState.test.ts` | MODIFY (race repro test + flushNow tests) |
| 2 | `apps/admin/src/state/useWorkspaceState.ts` | MODIFY (export `flushNow(slug)` + race fix per chosen approach J1) |
| 3 | `apps/admin/src/views/Shell.tsx` | MODIFY (wire `onModEnter` to `flushNow(slug)` when in editor route) |
| 4 | `tests/integration/apps/admin/resort-editor-write.test.tsx` | MODIFY (bridge integration assert: clear-during-flight no longer leaves blank) |

(4 files. PR 4.6c may also need to bump Shell.tsx for the `onModEnter` wiring — counted above. Subagent Review trigger TBD per chosen approach J1.)
...
**Branch:** `epic-4/pr-4.6c-workspace-race-fix`. **Depends on:** PR 4.6a merged. **Subagent Review trigger:** TBD per Decision J1 (if PUT-API widening is chosen, `packages/schema/**` is touched → YES; if AbortController-only, `apps/admin/src/state/**` only → NO).

- **Task 4.6c-0**: Brainstorm with `superpowers:brainstorming` — pick race-fix architectural approach J1 (AbortController vs PUT-API widening). Document choice in this plan (append a Decision J1-resolved row).
- **Task 4.6c-1**: Write failing race-repro test in `useWorkspaceState.test.ts`: type valid value → debounce starts PUT → user clears input mid-PUT → assert workspace file does NOT persist the in-flight value (or assert the response's prepopulate is suppressed).
- **Task 4.6c-2**: Write failing `flushNow(slug)` test: edit → call `flushNow(slug)` → assert PUT fires immediately (without 500 ms debounce).
- **Task 4.6c-3**: Implement the race fix per J1.
- **Task 4.6c-4**: Implement `flushNow(slug)` export.
- **Task 4.6c-5**: Modify Shell.tsx — wire `onModEnter: () => { if (route.route === 'editor') { flushNow(route.slug) } }`. (Note: `route` from `useURLState()`.)
- **Task 4.6c-6**: Bridge integration test in `resort-editor-write.test.tsx` (or new file): clear-during-flight no longer leaves the workspace file with a stale value AND the editor's input shows the cleared state correctly.
- **Task 4.6c-7**: qa green + push + Subagent Review (if J1-PUT-API path) + Codex review loop + local-test plan + final report.
