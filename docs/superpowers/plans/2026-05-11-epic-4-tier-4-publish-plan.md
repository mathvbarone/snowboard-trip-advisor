...
| **G1** | Shell's HeaderBar gains a `<Button>Publish</Button>` (existing DS primitive, `variant="primary"` per design pattern). Click toggles `isPublishOpen` local Shell state. `<ToastProvider>` wraps Shell's main content. PublishDialog accepts **`{open, onOpenChange}` props** per Decision F1's DS-Modal contract (Codex round 17 PR #97 P2 fold — Modal is controlled). **Conditional mount preserved** per Codex round 3 PR #97 P1 fold: `{isPublishOpen && <PublishDialog open={isPublishOpen} onOpenChange={setIsPublishOpen} />}`. Each open is a fresh mount that re-runs `useHealth()`'s effect against the latest health snapshot (without conditional mount, `useHealth`'s empty-dep useEffect would fire once at app boot and never re-fetch). Codex round 19 PR #97 P2 fold removed the obsolete claim that PublishDialog "no longer accepts an `open` prop" — it does, alongside `onOpenChange`. | Locality + freshness + DS contract. Conditional mount + Modal's controlled API compose cleanly. |
...
- **Server (Phase 1 — no header plumbing yet):** `DispatchInput` (`apps/admin/server/dispatch.ts:27-32`) only carries `{method, pathname, search, body}` — there is NO `headers` field, and the Vite middleware + MSW bridge call `dispatch()` without forwarding HTTP headers (Codex round 16 PR #97 P2 fold corrected the original J1 server claim, which assumed plumbing that doesn't exist). Phase 1 therefore "honors" `Idempotency-Key` at the HTTP boundary (the dev server simply ignores unknown headers — no 400 response) without the application server ever observing it. The publish handler does NOT see, log, or dedup on the header in Phase 1. **No dispatch-test case** is feasible because the dispatcher can't observe the header in the first place. Phase 2's Hono service swap will add header plumbing through `DispatchInput.headers` (or equivalent middleware context) when implementing real dedupe; the wire contract stays unchanged from the client's perspective.
...
- **No new DS `Modal` primitive — consume the EXISTING one.** PublishDialog wraps the existing `packages/design-system/src/primitives/Modal.tsx` (Radix-backed) per Decision F1 (Codex round 17 PR #97 P2 fold). Adding a new Modal would duplicate functionality + violate AGENTS.md no-raw-HTML.
...
- **No `useResortDetail()` invalidation on publish success.** Workspace files aren't mutated by publish; per-resort caches stay valid. Only invalidate `listPublishes` (Codex round 16 PR #97 P2 fold — `health` invalidation was originally listed but Decision D3 was dropped in round 2 PR #97 fold; `useHealth` is no longer extended, so there's nothing to invalidate).
...
// Codex round 16 PR #97 P2 fold: removed "honors Idempotency-Key header" test —
// DispatchInput (apps/admin/server/dispatch.ts:27) has no headers field, so the
// dispatcher cannot observe the header. The Phase-1 "honor without enforcing"
// happens at the HTTP boundary (Vite ignores unknown headers); no server-side
// test is feasible until Phase 2 adds header plumbing. See Decision J1 server
...
export function PublishDialog({ open, onOpenChange }: PublishDialogProps): JSX.Element {
  ...
  // The prior shape only described the blocker case, leaving AT users without
  // any explanation when health was loading or had failed.
    ...
    <Modal open={open} onOpenChange={onOpenChange} title="Publish">
      ...
      <div className="publish-dialog__actions">
        ...
        <Button
          ...
          disabled={disabled}
          aria-describedby={hasBlockerCopy ? 'publish-dialog-blocker' : undefined}
        >
