...
**Branch base — see [AGENTS.md `## PR Sizing Discipline`](../../../AGENTS.md#pr-sizing-discipline) (L108).** AGENTS.md is now the canonical source: stacking with `--base <foundation-branch>` is the documented default for hard dependency order, and the `delete_branch_on_merge: false` repo setting prevents the phantom-merge condition mechanically. Sequential-rebase (branching from `main` only after each predecessor merges) is a valid alternative documented in the same bullet. Pick whichever based on whether you want parallel review (stacked) or serialized (sequential-rebase).
