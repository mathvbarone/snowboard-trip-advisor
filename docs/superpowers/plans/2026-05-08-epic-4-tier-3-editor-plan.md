...
| **F1** | MANUAL edit input scope: **DS `Input type="text"`** (per round-15 P2-20: raw `<input>` JSX banned in apps/admin/src/**; DS Input ships only `'text' \| 'date'` per `packages/design-system/src/components/Input.tsx:20`) with **JS-side numeric/range/integer validation** for the **7 durable numeric metric paths** (`altitude_m.min`, `altitude_m.max`, `slopes_km`, `lift_count`, `skiable_terrain_ha`, `season.start_month`, `season.end_month`). The 5 live paths (`snow_depth_cm`, `lifts_open.count`, `lifts_open.total`, `lift_pass_day`, `lodging_sample.median_eur`) render explanatory copy unconditionally: "MANUAL editing for `${path}` lands in PR 4.6a." (per round-22 P2-30 — does NOT require MANUAL flip; the toggle is `disabled` for live paths). Months: input still text; JS validates 1–12 + integer; display formatter renders English month names (asymmetry documented). Gate test uses `slopes_km` (durable, numeric). | Tier 3 → 4 gate only requires MANUAL round-trip on one numeric path. **The cross-key invariant in `WorkspaceFile` (per spec §10.2) restricts `editor_modes` keys to `Object.keys(resort.field_sources)` — i.e., durable paths only. Trying to PUT `editor_modes: { snow_depth_cm: 'manual' }` would fail the refinement and 400 as `invalid-resort`.** Codex round-1 P2-1 fold corrected the v1 plan's "10 numeric paths" claim. Round-15 P2-20 fold corrected the input element from raw `<input type="number">` to DS `Input type="text"`. Round-22 P2-30 fold added the `disabled` toggle + unconditional copy for live paths. Round-23 P3-33 fold updated this row to reflect the actual DS-Input approach (the row's previous wording still pointed at the abandoned raw-`<input type="number">` design). Phase-2 widens the schema if live paths need MANUAL. |
...
export function clearFieldValue(slug: ResortSlug, path: MetricPath): void {
  ...
  const side = sideFor(path)
  const segments = path.split('.')
  // Per Codex round-24 P1-34 fold: for nested paths, clearDraftLeaf drops
  // the WHOLE parent value AND all siblings under the parent in
  // field_sources. Sibling statuses must also be cleared so the analyst
  // doesn't see stale 'dirty'/'saving' indicators on sibling fields whose
  // draft entries are now gone.
  const parentPrefix = segments.length === 1 ? null : segments[0] + '.'
  patchState(store, (s) => {
    const draft = clearDraftLeaf(s.draft, side, path)
    const status = { ...s.status }
    for (const [statusPath, statusValue] of Object.entries(s.status)) {
      const isClearedPath = statusPath === path
      const isSiblingUnderClearedParent = parentPrefix !== null && statusPath.startsWith(parentPrefix)
      if ((isClearedPath || isSiblingUnderClearedParent) && (statusValue === 'dirty' || statusValue === 'saving')) {
        delete status[statusPath as MetricPath]  // back to "no local edit pending"
      }
    }
...
function clearDraftLeaf(draft: DraftShape, side: Side, path: MetricPath): DraftShape {
  ...
  // Also drop field_sources entries. For top-level paths: just the
  // edited path's entry. For nested paths: drop ALL siblings under the
  // same parent (per Codex round-24 P1-34 fold) — round-21 P2-29 drops
  // the WHOLE parent value, so leaving a sibling's manual `field_sources`
  // entry behind would PUT manual provenance attached to the canonical
  // (now-unchanged) value, falsely attributing the canonical value to a
  // manual edit on the server's deep-merge.
  const fs = next.field_sources as Record<string, unknown> | undefined
  if (fs !== undefined) {
    const nextFs: Record<string, unknown> = {}
    const parentPrefix = segments.length === 1 ? null : segments[0] + '.'
    for (const [fsPath, fsEntry] of Object.entries(fs)) {
      if (fsPath === path) { continue /* the cleared leaf — drop */ }
      if (parentPrefix !== null && fsPath.startsWith(parentPrefix)) {
        continue /* sibling under the same dropped parent — drop too */
      }
      nextFs[fsPath] = fsEntry
    }
...
export function useModeToggle() {
  ...
  function canonicalModeFor(path: MetricPath): 'manual' | 'auto' {
    // Per Codex round-23 P2-32 fold: `field_states` is
    // `z.partialRecord(z.enum(METRIC_FIELDS), FieldStateFor)` per the API
    // contract — under `noUncheckedIndexedAccess`, `field_states[path]` is
    // `FieldStateFor<unknown> | undefined`. Reading `.state` directly would
    // crash if a canned/bridge response omits the entry. Optional-chain to
    // 'auto' fallback keeps the canonical-mode derivation safe.
...
- [ ] **Step 2: Add failing tests** to `FieldRow.test.tsx`. Three new branches (each test calls `stubMatchMedia(true)` or `stubMatchMedia(false)` per its viewport intent):
  ...
  - **Edit-then-clear drops value/provenance from the pending PUT** per Codex round-20 P2-28 + round-24 P2-35 folds: above-md MANUAL on `lift_count`. **Setup**: BEFORE typing, the analyst flipped the toggle to MANUAL — so `editor_modes['lift_count'] = 'manual'` is in the draft. Type `'7'` (valid) → setFieldValue fires; `draft.resort.lift_count = 7` + `draft.resort.field_sources.lift_count = manual`; debounce timer set. BEFORE 500ms elapses, type `''` (clear) — `clearFieldValue('lift_count')` fires; `draft.resort.lift_count` is removed; `draft.resort.field_sources.lift_count` is removed; status reverts to no-edit-pending. **`draft.editor_modes['lift_count']` is preserved** (per round-20 P2-28). Fast-forward debounce. **Assert** (per Codex round-24 P2-35 — corrects the round-20 over-broad assertion): the pending PUT IS sent (mode-only — the editor_modes flag is still pending), BUT the body has NO `resort.<value>` or `resort.field_sources.<path>` entry — body is `{ editor_modes: { lift_count: 'manual' } }`. The earlier "no PUT at all" assertion was incompatible with the editor-modes preservation. Same flow for invalid intermediates (`'7'` → `'7e'` → cleared) — body has only the editor_modes entry. **No-mode variant**: if the user typed `'7'` then cleared WITHOUT first flipping MANUAL (so `editor_modes` was untouched), `apiClient.upsertResort` is NOT called (empty diff via round-18 P2-25 short-circuit).
  ...
  - **Edit-then-clear of a nested path drops the whole hydrated parent** per Codex round-21 P2-29 fold: above-md MANUAL on `season.start_month`. Canonical has `season: { start_month: 12, end_month: 4 }`. Type `'11'` (valid) → setFieldValue fires; `patchDraftLeaf` hydrates parent: `draft.resort.season = { start_month: 11, end_month: 4 }` (sibling preserved per round-4 P2-6). BEFORE debounce, type `''` (clear) → `clearFieldValue('season.start_month')` fires. **Assert: `draft.resort?.season` is `undefined`** (whole parent dropped — NOT `{ end_month: 4 }`). Fast-forward debounce → `apiClient.upsertResort` is NOT called (when no `editor_modes` is pending). Without this fix, the next flush would PUT `resort: { season: { end_month: 4 } }`; server's shallow-merge of `Resort.season` would replace whole, leaving `season: { end_month: 4 }` missing `start_month` → fails `Resort.parse()` → `400 invalid-resort`. Repeat the same flow for `altitude_m.min` (canonical 1500/2000) — assert `draft.resort?.altitude_m` is `undefined` after clear.
  - **Nested-clear ALSO drops sibling field_sources entries** per Codex round-24 P1-34 fold: above-md MANUAL on BOTH `altitude_m.min` AND `altitude_m.max`. Type `'1500'` for `altitude_m.min` → setFieldValue + manual field_sources. Type `'2200'` for `altitude_m.max` → setFieldValue + manual field_sources. Now `draft.resort.altitude_m = { min: 1500, max: 2200 }` AND `draft.resort.field_sources['altitude_m.min']` + `draft.resort.field_sources['altitude_m.max']` both have manual entries. Clear `altitude_m.min` → `clearFieldValue('altitude_m.min')` fires. **Assert**: (a) `draft.resort?.altitude_m` is `undefined` (parent dropped per round-21); (b) **BOTH** `draft.resort.field_sources['altitude_m.min']` AND `draft.resort.field_sources['altitude_m.max']` are removed (per round-24 — sibling provenance must NOT remain orphaned, otherwise the next PUT would carry sibling's manual provenance attached to the canonical altitude_m.max value, falsely attributing it to a manual edit on the server's deep-merge); (c) statuses for both `altitude_m.min` AND `altitude_m.max` are no longer `dirty`/`saving` (sibling status also cleared so the analyst doesn't see stale indicators).
...
### Codex round 23 (PR #90 review by `chatgpt-codex-connector`, 2026-05-09)

Two findings on the v24 plan (one P2, one P3); both folded:

- **P2-32 — `useModeToggle.canonicalModeFor` would TypeScript-crash under `noUncheckedIndexedAccess`.** `ResortDetailResponse.field_states` is `z.partialRecord(z.enum(METRIC_FIELDS), FieldStateFor)` per the API contract — meaning entries can be missing. Under the project's strict TypeScript config (`noUncheckedIndexedAccess`), `field_states[path]` is typed as `FieldStateFor<unknown> | undefined`. v24's `detail.field_states[path].state === 'manual'` would (a) fail typecheck and (b) crash at runtime if a canned/bridge response omits a metric. **Fold:** added optional chaining: `detail.field_states[path]?.state === 'manual' ? 'manual' : 'auto'`. Falls back to `'auto'` when the entry is missing — same default as the no-draft / no-canonical-manual case.
- **P3-33 — Decisions log F1 row was stale.** F1 still described raw `<input type="number">` for the 7 durable numeric paths and `min/max` browser-attribute validation for months — but rounds 15 P2-20 + 22 P2-30 had already swapped to DS `Input type="text"` with JS-side validation, and added the `disabled` toggle + unconditional explanatory copy for live paths. Since AGENTS.md / project discipline says "follow the Decisions log first," a stale F1 row could send executors back to the rejected design. **Fold:** F1 rewritten to reflect the current authoritative design: DS `Input type="text"`, 7 durable numeric paths, JS validation (numeric/range/integer/finite), live paths render explanatory copy + disabled toggle. Cross-references all relevant Codex rounds (P2-1, P2-20, P2-30, P3-33).

### Codex round 24 (PR #90 review by `chatgpt-codex-connector`, 2026-05-09)

Two findings on the v25 plan (one P1, one P2); both real correctness issues; both folded:

- **P1-34 — Nested-leaf clear left sibling field_sources orphaned.** v25's `clearDraftLeaf` for nested paths dropped the WHOLE parent value (round-21 P2-29) but only removed the cleared path's `field_sources` entry. If the user had edited BOTH leaves of the same parent (e.g., `altitude_m.min` AND `altitude_m.max`) with manual provenance and then cleared one, the sibling's manual `FieldSource` entry would remain in the draft. The next PUT body would carry `field_sources: { 'altitude_m.max': <manual> }` WITHOUT the value (parent dropped) — server's deep-merge would attach manual provenance to the unchanged canonical `altitude_m.max` value, **falsely attributing the canonical value to a manual edit**. **Fold:** `clearDraftLeaf` now drops ALL `field_sources` entries whose path starts with the cleared parent prefix (e.g., clearing `altitude_m.min` drops both `field_sources['altitude_m.min']` AND `field_sources['altitude_m.max']`). `clearFieldValue` similarly clears statuses for all sibling paths under the parent so the analyst doesn't see stale `dirty`/`saving` indicators. PR 4.4d Task 6 test list adds a "nested-clear ALSO drops sibling field_sources entries" case asserting both leaves' provenance is gone after clearing one.
- **P2-35 — Round-20 P2-28 test assertion conflicted with the editor_modes preservation guarantee.** The "edit-then-clear cancels the pending PUT" test required `apiClient.upsertResort` NOT to be called after type-then-clear. But round-20's fold ALSO preserved `draft.editor_modes[path]` (the analyst's MANUAL flag survives clear) — so if the user had toggled MANUAL before typing, the next flush body has `{ editor_modes: { path: 'manual' } }` (mode-only, NOT empty). The PUT DOES fire; the assertion would either fail OR pressure the implementer to also drop `editor_modes`, breaking the round-3 P1-1 reload-preserves-mode gate. **Fold:** corrected the assertion to "no value/provenance payload sent (body is mode-only)" — verifies `body.resort` and `body.live_signal` are undefined while permitting `body.editor_modes`. Added a "no-mode variant" sub-case: if the user did NOT toggle MANUAL before typing, the diff is fully empty and `apiClient.upsertResort` is NOT called (round-18 P2-25 short-circuit).
