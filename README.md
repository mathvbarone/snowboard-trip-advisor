...
- **`apps/admin`** — loopback-only editor (`127.0.0.1:5174`, `strictPort:true`). All six `/api/*` endpoints are wired with real handlers: `GET /api/health` (full aggregates including `resorts_with_corrupt_workspace`), `GET /api/resorts` (filterable + sortable), `GET /api/resorts/:slug` (workspace-precedence read with `field_states` projection per spec §4.2 / §4.2.1, including draft-resort `live_signal: null` semantics and the `workspace-corrupt` envelope with actionable `.details` payload), `PUT /api/resorts/:slug` (debounced workspace upsert with `AbortController`-gated in-flight-clear), `POST /api/resorts/:slug/publish` (atomic publish via `publishDataset()`), `GET /api/publishes` (paginated archive listing). UI surface: Dashboard view + ResortsTable view + ResortEditor (Tier 3) + PublishDialog with pre-publish blocking-state gating + PublishHistory view at `?route=publishes` (Tier 4) + Toast notifications + global keyboard shortcuts (`g r`, `g i`, `mod+enter`, `Escape`) + responsive read-only-below-`md` via native `disabled` on header action buttons (Tier 5). Tier 5 → Epic 4 done gate closed at PR 4.6c (spec §7.4); see [`docs/superpowers/handoffs/2026-05-12-post-epic-4.md`](docs/superpowers/handoffs/2026-05-12-post-epic-4.md) for the post-Epic-4 onboarding handoff.
- **Publish pipeline (library + admin endpoint)** — `@snowboard-trip-advisor/schema/node` exports `publishDataset()` + `validatePublishedDataset()` (parse → Zod validate → provenance check → atomic write → archive). The admin's `POST /api/resorts/:slug/publish` endpoint wires into this library (shipped in Tier 4 PR 4.5a). The pre-pivot `research/` directory and its scoring code were deleted in Epic 2; a `research/cli.ts` wrapping the same library is planned for Epic 5 alongside the first real upstream adapters.
...
apps/admin/              Admin loopback editor (Dashboard, ResortsTable, ResortEditor, PublishDialog, PublishHistory)
...
- **Phase 1** — in progress on `main`. Epics 1–4 (workspace scaffolding + schema migration + public app + admin app) shipped. Epics 5–6 (real adapters, ops) follow.
