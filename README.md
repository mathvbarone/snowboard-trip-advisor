...
- **`packages/design-system`** — hand-built design system: `Card`, `Button`, `Input`, `Select`, `Chip`, `Pill`, `SourceBadge`, `FieldValueRenderer`, `Modal`, `Drawer`, `ShortlistDrawer`, `ShareUrlDialog`, `Table`, `ToggleButtonGroup`, `Sidebar`, `StatusPill`, `Tabs`, `Popover`, `DropdownMenu`, `EmptyStateLayout`, `Skeleton`, generated `tokens.css`.
- **`apps/public`** — full Phase 1 cards landing + matrix view + detail drawer + URL-shared shortlist + share-URL dialog + merge/replace dialog + view toggle + `?highlight=` row affordance + bundle-budget tooling (`npm run analyze`).
- **`apps/admin`** — loopback-only editor (`127.0.0.1:5174`, `strictPort:true`). Real handlers for `GET /api/health` (full aggregates including `resorts_with_corrupt_workspace`) and `GET /api/resorts` (filterable + sortable). Dashboard view + ResortsTable view + Dashboard "Failed fields" card-click navigation. Editor view + write path land in Tier 3.
- **Publish pipeline** — runs in-process inside admin's Vite middleware via `publishDataset()` + `validatePublishedDataset()` from `@snowboard-trip-advisor/schema/node` (parse → Zod validate → provenance check → atomic write → archive). The pre-pivot `research/` directory and its scoring code were deleted in Epic 2; a `research/cli.ts` wrapping the library is planned for Epic 5 alongside the first real upstream adapters.
