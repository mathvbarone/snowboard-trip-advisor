...
- **`apps/admin`** — loopback-only editor (`127.0.0.1:5174`, `strictPort:true`). Real handlers for `GET /api/health` (full aggregates including `resorts_with_corrupt_workspace`), `GET /api/resorts` (filterable + sortable), and `GET /api/resorts/:slug` (workspace-precedence read with `field_states` projection per spec §4.2 / §4.2.1, including draft-resort `live_signal: null` semantics and the `workspace-corrupt` envelope with actionable `.details` payload); the remaining 3 endpoints (`resortUpsert`, `publish`, `listPublishes`) are 501-stubs that ship in Tier 3 / Tier 4. Dashboard view + ResortsTable view + Dashboard "Failed fields" card-click navigation. Editor view + write path land in Tier 3; publish handler lands in Tier 4 PR 4.5a.
