...
// Pure, browser-safe variant of loadResortDataset. Takes already-parsed
// `unknown` (e.g. from `await response.json()` in apps/public's datasetFetch.ts);
// runs validatePublishedDataset, then projects the validated dataset into
// ResortView shape. No node:fs/promises — safe to bundle in apps/public.
//
// Returns a Promise (not a sync value) so the public surface matches the
// Node wrapper's `Promise<LoadResult>` signature; both call sites await.
// The function body is synchronous (no I/O); we materialize the Promise via
// `Promise.resolve` and skip `async` to satisfy require-await lint.
export function loadResortDatasetFromObject(
  raw: unknown,
  { now = new Date() }: LoadOptions = {},
): Promise<LoadResult> {
  const validation = validatePublishedDataset(raw)
  if (!validation.ok) {
    return Promise.resolve({ ok: false, issues: validation.issues })
  }
  ...
  return Promise.resolve({ ok: true, views })
}
