...
- **Stacked PRs over bundled PRs.** When concerns have a hard dependency order (e.g. design-system primitive → consumer view), open the foundation PR first, then open the dependent PR with `--base <foundation-branch>` so reviewers see only the dependent's diff. When the foundation merges, the dependent's diff against `main` becomes correct automatically. The repo's `delete_branch_on_merge` setting is **OFF** (verify via `gh api repos/<owner>/<repo> --jq '.delete_branch_on_merge'`), which prevents the phantom-merge condition described below — stacking is mechanically safe under the current configuration. Sequential-rebase (branching from `main` only after the predecessor merges) is a valid alternative if the implementer prefers serialization over parallel review.
...
**Stacked-PR phantom-merge — historical incident + recovery procedure (rare under current config).** Twice in the project's history, stacked PRs were marked `MERGED` by GitHub even though their actual commits never landed on `main`: PR [#56](https://github.com/mathvbarone/snowboard-trip-advisor/pull/56) (Epic 3 close — recovered as PR [#57](https://github.com/mathvbarone/snowboard-trip-advisor/pull/57)) and PRs [#58](https://github.com/mathvbarone/snowboard-trip-advisor/pull/58)–[#62](https://github.com/mathvbarone/snowboard-trip-advisor/pull/62) (agent-discipline migration — recovered as PR [#66](https://github.com/mathvbarone/snowboard-trip-advisor/pull/66)). **Root cause:** when stacking via `gh pr create --base <other-branch>` and GitHub auto-deletes the parent branch on merge, GitHub treats the stacked PR's now-orphaned base as merged. **Prevention in place:** the repo's `delete_branch_on_merge: false` setting (see the bullet above) removes the trigger condition; stacked PRs under the current configuration cannot phantom-merge from auto-deletion alone. **Recovery procedure** — apply only if (a) `delete_branch_on_merge` is ever flipped back ON, OR (b) a stacked PR's parent branch is manually deleted before the stacked PR merges: run `gh pr view <stacked-PR> --json baseRefName,mergeCommit` after the parent PR merges; if `baseRefName` points to a now-deleted branch and `mergeCommit.oid` is not reachable from `main`, the diff was lost — re-apply with `git cherry-pick` onto a fresh branch off `main`. GitHub's "MERGED" badge on a stacked PR after its base is deleted is a false positive in this scenario. See [Epic 3 spec §10.8](docs/superpowers/specs/2026-04-28-epic-3-public-app-design.md#108-pr-56-phantom-merge-incident-epic-3-close) for the PR #56 worked example; commit [652d20b](https://github.com/mathvbarone/snowboard-trip-advisor/commit/652d20b) (PR #66) for the PRs #58–#62 recovery.
